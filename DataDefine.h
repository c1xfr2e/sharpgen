#ifndef __PONOVO_DATA_DEFINE_H__
#define __PONOVO_DATA_DEFINE_H__

#include <QString>
#include <QStringList>
#include <QVector>

#define MAX_EQUIPMENT_COUNT   8
#define SAMPLE_MODEL_LEN      32
#define COMMON_NAME_LEN       128
...
typedef enum tagEnumCatalog
{
    ...
    EC_FUNCTIONAL_TEST,                       // 功能检查
    EC_SAFTY_REQUIREMENT,                     // 安全要求检查
    EC_CHARGING_MODE_AND_CONN_METHOD,         // 充电模式和连接方式检查
    EC_CONN_DEV_AND_CABLE_CHECK,              // 充电连接装置及电缆检查
    EC_ELECTRICAL_ISOLATION_CHECK,            // 电气隔离检查
    EC_ELECTRIC_SHOCK_PROTECT,                // 电击防护实验
    EC_CREEPAGE_DISTANCES_AND_CLEARANCES_CHK, // 电气间隙和爬电距离
    ...
    EC_INTEROPERABILITY_EXPERIMENT            // 互操作性实验
}EnumCatalog;
...
typedef enum tagEnumChargingMode
{
    CHARGING_MODE_1 = 0,
    CHARGING_MODE_2,
    CHARGING_MODE_3,
    CHARGING_MODE_4
}EnumChargingMode;   // GB/T 18487.1-2015, page 9[5.1]

typedef enum tagEnumGunConnMode
{
    GUN_CONNECT_A_MODE = 0,
    GUN_CONNECT_B_MODE,
    GUN_CONNECT_C_MODE
}EnumGunConnMode;   // GB/T 18487.1-2015, page 3[3.1.3]

typedef enum tagUnitType
{
    UT_INVALID,
    CUR_mA,
    CUR_A,
...
typedef enum tagEnumUsageCircumstance
{
    EUC_INDOOR_USE,
    EUC_OUTDOOR_USE
}EnumUsageCircumstance;   // GB/T 18487.1-2015, page 7[3.5]

typedef enum tagEnumTestEquipmentType
{
    TET_TEST_DC_CHARGING_PILE_DEV,
    TET_TEST_AC_CHARGING_PILE_DEV,
    TET_DIGITAL_MULTIMETER,
    TET_UNKNOWN_DEV
}EnumTestEquipmentType;

typedef struct tagSelfVar
{
    float    val;
    UnitType uType;
}SelfVar;

typedef struct tagSelfVarRange
{
    float    low;
    float    high;
    UnitType uType;
...
typedef struct tagVarRange
{
    float    low;
    float    high;
}VarRange;

typedef struct tagBdDateTime
{
    short Year;
    char  Month;
    char  Day;
...
typedef enum tagEnumCheckStage
{
    CS_UNCHECK = 0,
    CS_CHECKING,
    CS_CHECKED
...
typedef struct tagInspectorInfo
{
    char testerName[COMMON_NAME_LEN];        // test operator Name
    char auditName[COMMON_NAME_LEN];         // audit Name
    char approverName[COMMON_NAME_LEN];      // approver Name
}InspectorInfo;

typedef struct tagTestSampleInfo
{
    char sampleId[SAMPLE_MODEL_LEN];         // Sample id or Applicant ticket id
    char sampleModel[SAMPLE_MODEL_LEN];      // Test Sample model
    char sampleName[COMMON_NAME_LEN];        // Test Sample name( id or SN )
    char manufacturerName[ COMMON_NAME_LEN]; // Manufacturer Name
    char assetsNo[COMMON_NAME_LEN];          // Asset tag or asset number
    BdDateTime manufacturerDate;             // sample Manufacturer Date
}TestSampleInfo;

typedef struct tagReportBaseInfo
{
    char projectName[COMMON_NAME_LEN];        // Project Name
    char applicantTicketId[SAMPLE_MODEL_LEN]; // Applicant ticket id
    char applicantName[ COMMON_NAME_LEN];     // Applicant Name
    char testAddress[COMMON_NAME_LEN];        // Test Address
    EnvironmentInfo envInfo;                  // Environment Info
    BdDateTime testDate;                      // test Date and time
    BdDateTime signDate;                      // report sign Date
}ReportBaseInfo;

typedef struct tagChargingPileBaseInfo
{
    char namePlate[COMMON_NAME_LEN];    // name plate id
    char model[COMMON_NAME_LEN];        // Charging-Pile model
    float ratedInputVolt;               // rated Input Volt of Charging-Pile
    float maxOutputCur;                 // max Output current of Charging-Pile
    float outputVoltRange;
    float ratedOutputPower;             // unit:KW
    EnumUsageCircumstance usageCircumstance;  // Charging-Pile usage Circumstance
    BdDateTime BuildDate;
}ChargingPileBaseInfo;

typedef struct tagTestEquipmentItem
{
    EnumTestEquipmentType devType;
    char      devName[COMMON_NAME_LEN];
    char       devId[SAMPLE_MODEL_LEN];
    BdDateTime expireDate;   // device quality guarantee Date
}TestEquipmentItem;


typedef enum tagEnumPackingMode
{
    EPM_CUSTOM_BOX    = 0,
    EPM_OTHER    = 1
}EnumPackingMode;

typedef struct tagPackingMode
{
    EnumPackingMode  mode;
    char             otherDesc[COMMON_NAME_LEN];
}PackingMode;

typedef enum tagEnumShipMode
{
    ESM_TRUCK    = 0,
    ESM_TRAIN    = 1,
    ESM_FLIGHT   = 2,
...
typedef struct tagShipMode
{
    EnumShipMode  mode;
    char          otherDesc[COMMON_NAME_LEN];
}ShipMode;

typedef enum tagEnumDevKeepStatus
{
    EDKS_NORMAL    = 0,
    EDKS_ABNORMAL  = 1
}EnumDevKeepStatus;

typedef struct tagSystemDevInfo
{
    ChargingPileBaseInfo Pile;
    int EquipmentCnt;
    TestEquipmentItem Equipment[MAX_EQUIPMENT_COUNT];
    PackingMode packing;
    ShipMode    shipping;
    EnumDevKeepStatus devStatus;
    EnumDevKeepStatus connLineStatus;
}SystemDevInfo;




bool ReadUserListFromByteArray(const QByteArray& src, QVector<UserItem>& input );

void WriteUserListToByteArray( QByteArray& dest, const QVector<UserItem>& input );
